...
watch(
    () => [props.open, props.system] as const,
    ([open, system]) => {
        if (!open || system === null) {
            return;
        }

        form.organization_id = system.organization_id;
        form.site_id = system.site_id;
        form.department_id = system.department_id;
        ...
        form.version = system.version ?? '';
        form.operating_system = system.operating_system ?? '';
        form.operating_system_version = system.operating_system_version ?? '';
        form.serial_number = system.serial_number ?? '';
        form.inventory_number = system.inventory_number ?? '';
...
const submit = (): void => {
    ...
    form.put(`/systems/${props.system.public_id}`, {
        preserveScroll: true,
        onSuccess: close,
    });
};
            ...
            <aside class="absolute inset-y-0 right-0 flex w-full max-w-xl flex-col bg-white shadow-2xl">
                <header class="flex items-start justify-between border-b border-slate-200 px-6 py-5">
                    <div>
                        ...
                        <h2 id="edit-system-title" class="mt-1 text-xl font-semibold text-slate-950">
                            System bearbeiten
                        </h2>
